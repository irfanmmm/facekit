...
def run_merge():
    ...
    with open('duplicate_faces.csv', 'r') as f:
        ...
        for row in reader:
            ...
            # Merge OfficeKit SQL Attendance
            if sql_conn:
                cursor = sql_conn.cursor()
                try:
                    cursor.execute("""
                        UPDATE ATTENDANCELOG_STAGING 
                        SET UserId = %s 
                        WHERE UserId = %s
                    """, (primary_code, duplicate_code))
                    sql_conn.commit()
                except Exception as e:
                    logger.error(f"SQL update failed for {duplicate_code} -> {primary_code}: {e}")
                    sql_conn.rollback()
                    
            logger.info(f"Successfully merged {duplicate_code} into {primary_code}")
